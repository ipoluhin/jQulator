...
const buttonsBase = {
	...
	exponentiation: [{ text: "&#8730x", class: "btn sqrt", id: "sqrt" }],
	digits: [
...
const htmlRender = {
	...
	themesRender: function () {
		for (let i = 0; i < buttonsBase.themes.length; i++) {
			$(".themes").html(
				$(".themes").html() +
					`<button class="${buttonsBase.themes[i].class}" id="${buttonsBase.themes[i].id}">${buttonsBase.themes[i].text}</button>`
			);
		}
	},
	/**
	...
	roundRender: function () {
		for (let i = 0; i < buttonsBase.round.length; i++) {
			$(".round-function").html(
				$(".round-function").html() +
					`<button button type = "button" class= "${buttonsBase.round[i].class}" id = "${buttonsBase.round[i].id}" >
	...
	memoryRender: function () {
		for (let i = 0; i < buttonsBase.memory.length; i++) {
			$(".memory-function").html(
				$(".memory-function").html() +
					`<button type="button" class="${buttonsBase.memory[i].class}" id = "${buttonsBase.memory[i].id}">
...
const round = {
	toNormal: function () {
		...
		if (vars.numAfterDot !== lastNumAfterDot) {
			$(".round-vol-active").removeClass("round-vol-active");
			$("#normal").addClass("round-vol-active");
			if (Number.isInteger(vars.result)) {
				$("#result-text").text("").text(vars.result);
			} else {
				$("#result-text")
					.text("")
...
const memory = {
	ms: function () {
		...
		$("#result-text").html('<span style="color: blue; font-size: 16px">saved</span>');
		setTimeout(() => {
			$("#result-text").text("");
		}, 1500);
	},
	mr: function () {
		if (+vars.memory === 0 || +vars.memory === "empty") {
			$("#result-text").html('<span style="color: blue; font-size: 16px">empty slot</span>');
			setTimeout(() => {
				$("#result-text").text("");
			}, 1500);
		} else {
	...
	mc: function () {
		vars.memory = 0;
		$("#input-text").text("");
		$("#result-text").html('<span style="color: blue; font-size: 16px">cleared</span>');
		setTimeout(() => {
			$("#result-text").text("");
		}, 1500);
	},
